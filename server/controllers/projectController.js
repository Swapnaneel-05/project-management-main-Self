...
export const createProject =  async (req, res)=>{
    try {
        ...
        const {workspaceId, description, name, status, start_data, end_date, team_members, team_lead, progress, priority} = req.body;

        //check if user is admin for workspace
        ...
        const project = await prisma.project.create({
            data: {
                ...
                start_date: start_data ? new Date(start_data) : null,
...
export const updateProject =  async (req, res)=>{
    try {
        const {userId} = await req.auth();
        const { id, workspaceId, description, name, status, start_data, end_date, progress, priority} = req.body;
...
export const addMember =  async (req, res)=>{
    try {
        ...
        if(project.team_lead !== userId){
            return res.status(404).json({ message: "Only team lead can add members"});
        }
